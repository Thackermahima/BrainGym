  ...
  return (
    <div className="flex flex-shrink-0 items-center text-xs font-medium text-jacarta-500 dark:text-jacarta-300 sm:text-sm">
      {menuItems.map((menuItem) => (
        <div
          key={menuItem.id}
          className={`flex h-10 w-full cursor-pointer items-center whitespace-nowrap border ${activeMenu === menuItem.id
              ? "border-transparent bg-accent text-white"
              : "border-jacarta-100 bg-white dark:border-jacarta-600 dark:bg-jacarta-700"
            } p-3 ${menuItem.id === "all-time" ? "first:rounded-l-lg" : ""} ${menuItem.id === "24h" ? "last:rounded-r-lg" : ""
